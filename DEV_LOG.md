...
**结算弹窗增强:**
- 新增「📰 事件结果」区域，展示事件后果解释
- 真实历史事件来源以「这是真实历史事件」可点击链接呈现（非裸 URL）
- 新增 `sourceUrl` 字段支持
- 支持中英文翻译

**事件描述全面改写:**
- 所有 20 个事件描述改为模棱两可风格，玩家无法从新闻判断涨跌
- 每个描述呈现正反两面观点，如同真实新闻客观报道

**像素风反馈弹窗 (PixelToast):**
- 替换所有浏览器原生 alert 为自定义像素风 Toast
- 买入成功 → 绿色提示 / 余额不足 → 红色提示
- 顶部滑入，2 秒自动消失

**价格信息板块 (Header) 美化:**
- 添加像素风背景图 (`public/header-bg.jpg`)
- 文字改为白色/浅灰适配暗色背景

**交易面板移动端适配:**
- 第一次点击 Buy/Sell → 展开百分比面板
- 点击百分比 → 按余额/持仓百分比交易
- 第二次点击 Buy/Sell → 按 Amount 输入的数量交易
- 点击外部区域 → 收起面板
- 桌面端 hover 行为保持不变
